document.addEventListener("DOMContentLoaded", () => {
  ...
  // lenis triggers updates to scrollTrigger whenever a scroll event occurs ensuring the animations remain perfectly synchronized
  const lenis = new Lenis();
  lenis.on("scroll", ScrollTrigger.update);
  gsap.ticker.add((time) => {
    lenis.raf(time * 1000);
  });
  //   gsap.ticker.logSmoothing(0);
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        const titles = Array.from(slides).map((slide) => {
          return slide.querySelector(".title h1");
        });
